...
//search the block txt file for a given name and return if it is inside the block or not.
int search_block(char* name){
    
    string line;
    ifstream block;
    block.open("block2.txt");
...
//returns the max serial number found on the block file
int find_max_serial()
{
    int max = 0;
    string line;
...
void post_to_txt(struct command cmnd){
       ...
       // printf("posting: %s \n",new_trans);
        ...
        finput.open ("block2.txt");
        foutput.open ("block2.txt",ios::app); 
...
void read_server(int udp_socket)
{
    ...
    printf("The ServerB recieved a request from the Main Server \n");
	//printf("%s\n", msg_buffer );
